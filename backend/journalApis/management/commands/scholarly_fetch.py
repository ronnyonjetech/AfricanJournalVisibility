...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        # Query journals without articles, ordered alphabetically by journal title (or customize the order)
        journals_without_articles = Journal.objects.filter(
            articles__isnull=True
        ).order_by('journal_title')  # Ensure journals are processed sequentially by title (you can change 'journal_title' to another field as needed)

        if not journals_without_articles.exists():
            self.stdout.write(self.style.WARNING("No journals found without articles."))
            return

        # Iterate over all journals without articles in order
        for journal in journals_without_articles:
            self.stdout.write(self.style.NOTICE(f"Processing journal: {journal.journal_title}"))
            ...
            try:
                ...
                for i, article in enumerate(search_query):
                    ...
                    # Introduce a fixed delay between requests to avoid overloading the server
                    self.stdout.write(self.style.NOTICE(f"Delaying next article request by {self.BASE_DELAY} seconds..."))
            ...
            # Fixed delay between journals to avoid throttling/blocking
            self.stdout.write(self.style.NOTICE(f"Delaying next journal request by {self.BASE_DELAY} seconds..."))
            time.sleep(self.BASE_DELAY)
